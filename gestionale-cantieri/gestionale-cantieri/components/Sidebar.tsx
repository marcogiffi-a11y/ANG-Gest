...
export default function Sidebar() {
  ...
  return (
    <aside style={{
      width: 200, background: '#0f172a', display: 'flex', flexDirection: 'column',
      flexShrink: 0, height: '100vh', position: 'fixed', left: 0, top: 0, zIndex: 50,
      overflowY: 'auto'
    }}>
      {/* Logo */}
      <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: 'white' }}>⚡ ANG Gest</div>
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>Athena Next Gen S.r.l.</div>
      </div>
        ...
        {navItems.map((item, i) => {
          ...
          return (
            <Link key={item.href} href={item.href} style={{
              ...
              fontSize: 12, color: active ? 'white' : 'rgba(255,255,255,0.48)',
              textDecoration: 'none',
