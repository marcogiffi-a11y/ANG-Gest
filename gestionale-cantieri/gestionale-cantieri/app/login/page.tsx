...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0f172a' }}>
      <div style={{ background:'white', borderRadius:16, padding:'40px 36px', width:380, boxShadow:'0 25px 60px rgba(0,0,0,0.3)' }}>
        {/* Logo */}
        <div style={{ textAlign:'center', marginBottom:28 }}>
          <div style={{ fontSize:28, marginBottom:6 }}>⚡</div>
          <div style={{ fontSize:18, fontWeight:800, color:'#0f172a' }}>ANG Gest</div>
        ...
        {sent ? (
          ...
          <>
            ...
            <div style={{ textAlign:'center', marginTop:14, fontSize:12, color:'#64748b' }}>
              {mode === 'login' ? (
                <>Non hai un account? <span style={{ color:'#3b82f6', cursor:'pointer', fontWeight:600 }} onClick={()=>setMode('signup')}>Registrati</span></>
              ) : (
                <>Hai già un account? <span style={{ color:'#3b82f6', cursor:'pointer', fontWeight:600 }} onClick={()=>setMode('login')}>Accedi</span></>
              )}
            </div>
          </>
        )}
