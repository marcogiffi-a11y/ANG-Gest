    ...
    <>
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
          {[
            { label: 'Ordini trovati',  value: filtered.length,                                       color: '#1e3a5f' },
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
          ...
          <Link href="/ordine/nuovo" style={{ padding: '8px 16px', borderRadius: 7, background: '#6ab04c', color: 'white', fontSize: 12, fontWeight: 600, textDecoration: 'none', whiteSpace: 'nowrap' }}>
            + Nuovo ordine
          </Link>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e5e7eb' }}>
                {['N° Ordine', 'Cliente', 'Portafoglio', 'Servizio', 'Importo netto', 'Importo lordo', 'SAL', 'Stato', 'Azioni'].map(h => (
            ...
            <tbody>
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
                  Nessun ordine trovato.{' '}<Link href="/ordine/nuovo" style={{ color: '#6ab04c' }}>Crea il primo →</Link>
                </td></tr>
              )}
              {filtered.map(o => {
                ...
                return (
                  <tr key={o.id} style={{ borderBottom: '1px solid #f1f5f9' }}
                    ...
                    <td style={{ padding: '11px 12px' }}>
                      <div style={{ fontWeight: 700, color: '#1e3a5f', fontFamily: 'monospace', fontSize: 12 }}>{o.numero_ordine}</div>
        ...
        {filtered.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10, gap: 24, fontSize: 12, color: '#64748b' }}>
            <span>{filtered.length} ordini</span>
