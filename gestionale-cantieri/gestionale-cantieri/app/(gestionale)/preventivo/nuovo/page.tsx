...
const SEZIONI_DEFAULT = [
  { label: 'A — Progettazione tecnica', voci: [
    ...
    'Studio di producibilità energetica + calcolo ombreggiamenti',
    'Relazione tecnica generale + As-Built a fine lavori',
  ]},
  { label: 'B — Iter autorizzativo e pratiche amministrative', voci: [
    'PAS ai sensi art. 6 D.Lgs. 28/2011',
    'Pratica SUAP / SUE presso il Comune competente',
    'Richiesta connessione Terna / E-Distribuzione',
    'Predisposizione documentazione accesso incentivi GSE',
    'Aggiornamento catastale post-intervento (DOCFA)',
  ]},
  { label: 'C — Preparazione cantiere', voci: [
    'Allestimento cantiere + recinzione perimetrale + cancelli',
    'Scavi e rinterri per cavidotti di cantiere',
    'Sistema di videosorveglianza perimetrale',
    'Impianto di illuminazione perimetrale a LED',
  ]},
  { label: 'D — Impianto FV lato DC', voci: [
    'Fornitura moduli fotovoltaici (monocristallini PERC/TOPCon)',
    'Strutture di appoggio moduli su pali infissi a terra',
    'Fornitura e installazione inverter di stringa/centralizzati',
    'Quadri di campo + cavi DC + cavidotti interrati lato DC',
  ]},
  { label: 'E — Impianto FV lato AC e media tensione', voci: [
    'Platea in c.a. + cabina Enel prefabbricata + cabina utente MT/BT',
    'Trasformatore MT/BT + quadro MT (QMT) + quadro BT (QBT)',
    'Cavi AC + cavidotti interrati lato AC + pozzetti ispezione',
  ]},
  { label: 'F — Direzione lavori, sicurezza e collaudo', voci: [
    'Direzione Lavori (DL) per tutta la durata del cantiere',
    'CSP + CSE ai sensi D.Lgs. 81/08',
    'Redazione PSC e POS',
    'Collaudo finale + Dichiarazione di Conformità D.M. 37/08',
    'Affiancamento prima registrazione GSE',
  ]},
]
...
export default function NuovoPreventivoPage() {
  ...
  async function salva() {
    ...
    try {
      ...
      // Inserisci voci (solo quelle con descrizione)
  ...
  return (
    <>
      <Topbar
        ...
        subtitle={step === 0 ? 'Scegli tipo servizio e tipo cliente' : `${tipoServizio === 'ingegneria' ? 'Ingegneria' : 'Fornitura/Posa'} · ${tipoCliente === 'privato' ? 'Privato' : tipoCliente === 'ente' ? 'Ente pubblico' : 'Altro soggetto'}`}
      />
        ...
        {step === 0 && (
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button
                onClick={procedi}
                disabled={!tipoServizio || !tipoCliente}
                style={{ padding: '8px 16px', borderRadius: 7, border: 'none', background: tipoServizio && tipoCliente ? '#6ab04c' : '#e2e8f0', color: tipoServizio && tipoCliente ? 'white' : '#94a3b8', fontSize: 12, fontWeight: 600, cursor: tipoServizio && tipoCliente ? 'pointer' : 'not-allowed' }}
              >
                Procedi →
              </button>
            </div>
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 14 }}>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20, background: tipoServizio === 'ingegneria' ? '#dbeafe' : '#fef3c7', color: tipoServizio === 'ingegneria' ? '#1d4ed8' : '#92400e' }}>
            ...
            <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e5e2', padding: 18, marginBottom: 12, position: 'relative' }}>
              ...
              {clienteSelezionato && (
                <div style={{ marginTop: 8, background: '#f0fdf4', border: '1px solid rgba(106,176,76,.3)', borderRadius: 6, padding: '8px 12px', fontSize: 12, color: '#166534' }}>
                  ✓ <strong>{clienteSelezionato.ragione_sociale}</strong> — P.IVA: {clienteSelezionato.piva} · {(clienteSelezionato as any).portafogli?.nome}
            ...
            {/* Voci */}
            <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e5e2', borderLeft: '3px solid #6ab04c', borderRadius: '0 10px 10px 0', padding: 18, marginBottom: 12 }}>
