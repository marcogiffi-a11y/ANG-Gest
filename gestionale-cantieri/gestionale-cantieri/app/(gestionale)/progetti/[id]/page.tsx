...
export default function DettaglioProgetto() {
  ...
  async function load() {
    ...
    setProgetto(p); setSals(s || []); setDocumenti(d || [])
    setLoading(false)
  }
  ...
  return (
    <>
      <Topbar title={progetto.clienti?.ragione_sociale || 'Dettaglio progetto'} subtitle={`${progetto.numero_ordine} · ${progetto.tipo_servizio === 'ingegneria' ? 'Ingegneria' : 'Fornitura e Posa'}`} />
      <div style={{ padding: '20px 24px' }}>

        {/* Header card */}
        <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e5e2', padding: 18, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#dbeafe', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: '#1d4ed8', flexShrink: 0 }}>
              {(progetto.clienti?.ragione_sociale || '??').slice(0, 2).toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#0f172a' }}>{progetto.clienti?.ragione_sociale}</div>
              <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>
                {progetto.clienti?.portafogli?.nome} · {progetto.numero_ordine}
                {progetto.numero_offerta && ` · Off. ${progetto.numero_offerta}`}
                {' · '}<span style={{ background: '#dbeafe', color: '#1d4ed8', fontSize: 10, fontWeight: 600, padding: '1px 7px', borderRadius: 20 }}>{progetto.tipo_servizio === 'ingegneria' ? 'Ingegneria' : 'Fornitura/Posa'}</span>
              </div>
              {progetto.servizi?.length > 0 && (
                <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {progetto.servizi.map((s: string) => <span key={s} style={{ fontSize: 10, background: '#f1f5f9', color: '#475569', padding: '2px 8px', borderRadius: 20 }}>{s}</span>)}
                </div>
              )}
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 12 }}>
          ...
          <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e5e2', padding: 18 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {sals.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', padding: 20, color: '#94a3b8' }}>Nessun SAL configurato</td></tr>}
                {sals.map(s => (
                  <tr key={s.id} style={{ borderBottom: '1px solid #fafafa' }}>
                    ...
                    <td style={{ padding: '8px 6px', color: '#64748b', fontSize: 11 }}>{s.data_prevista ? new Date(s.data_prevista).toLocaleDateString('it-IT') : '—'}</td>
                    <td style={{ padding: '8px 6px', fontWeight: 700 }}>{fmt(s.importo)}</td>
                    <td style={{ padding: '8px 6px' }}>
                      <select value={s.stato} onChange={e => aggiornaSalStato(s.id, e.target.value)} style={{
